...
# ─── Environment ───
class Td3TunerEnv(gym.Env):
    def __init__(self, X, y, seq_len, num_ranges, sup_h5_path, alpha=0.5):
        super().__init__()
        self.X, self.y       = X, y
        ...
        # load the model without compiling (avoids the missing 'mse' error)
        self.controller = tf.keras.models.load_model(sup_h5_path, compile=False)
        ...
        # action space = flatten(W) + flatten(b)
...
def main():
    ...
    p.add_argument('--sup_h5',     required=True,
                   help="Path to your supervised controller .h5")
    p.add_argument('--output_td3', default='Models/td3_mps.zip')
    ...
    _, num_ranges, _ = X.shape[1:]

    # build VecEnv
    base_env = DummyVecEnv([lambda: Td3TunerEnv(
        X, y, opts.seq_len, num_ranges,
        opts.sup_h5, alpha=opts.alpha
    )])
    ...
    print("✅ Saved MPS TD3 policy →", opts.output_td3)
